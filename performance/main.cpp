...
//In this branch, I use additional libraries for performance comparison.
...
class Specificator {}; // primary template
 
template<class Container>
class Specificator<Container,false> 
{
public:
	static void OutputPointer()
	{
	} 
	static int SumArray(Container &c)
	{
		return 0;
	}
...
template<class Container>
class Specificator<Container,true> 
{
public:
	static void OutputPointer()
	{
	...
	static int SumArray(Container &it)
	{
		int sum2=0;
...
void OutputPointer<true>(ofstream &ofs)
{
	int  (*func_var)(Container&)=&SumArray;
	void *ptr=(void*)func_var;
	cout<<ptr;
...
template <class Container,bool sequential_check>
void SingleOperationPerformanceCheck(ofstream &ofs,int elements,int maxsz)
{
	Specificator<Container,sequential_check>::OutputPointer();
	...
	while(lim<maxsz){
		...
		for(it=vecCon.begin();it!=vecCon.end();it++){
			sum2+=Specificator<Container,sequential_check>::SumArray(*it);
		}
	...
	cout<<"Our dummies "<<sum1<<" "<<sum2<<" "<<sum3<<"\n";
	...
	for(j=0;j<(log2-1);j++){
	    ...
	    if(sequential_check){
		   ofs<<" "<<setw(12)<<((t_readiter[j]-t_readrnd[j])*denom);
	    }
	    ofs<<"\n";
...
void PerformanceTest()
{
	TestDescriptor t1("Now performance tests.");
	{
		...
		ofs<<"vector<int>\n";
		SingleOperationPerformanceCheck<std::vector<int> ,true>(ofs,100000,1000);
		ofs<<"alex_kupriianov_array<int>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN> ,true>(ofs,100000,1000);
		ofs<<"vector<int>\n";
		SingleOperationPerformanceCheck<std::vector<int> ,true>(ofs,500,10000);
		ofs<<"deque<int>\n"; 
		SingleOperationPerformanceCheck<std::deque<int> ,true>(ofs,100,10000);
		ofs<<"alex_kupriianov_array<int>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN> ,true>(ofs,500,100000);
		ofs<<"alex_kupriianov_array<int, fast_memory_operator>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN,fast_memory_operator<int> > ,true>(ofs,100,100000);
		ofs<<"alex_kupriianov_array<int>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN> ,true>(ofs,1,10000000);
...
void AlexKupriianovArrayVsRopeTest()
{
	TestDescriptor t1("Comparison with rope.");
	{
		alex_kupriianov_array<int,MMM,NNN> akai;
		string s;
		std::ostringstream fname;
		fname<<"rope_vs_alex_kupriianov_array"<<MMM<<"_"<<NNN<<".txt";
		s=fname.str();
		ofstream ofs(s.c_str());
		ofs<<"Experiments with MMM="<<MMM<<" NNN="<<NNN
			<<" sizeof(Branch)="<<akai.__branch_size()
			<<" sizeof(Leaf)="<<akai.__leaf_size()<<"\n";
		ofs<<"alex_kupriianov_array<int>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN> ,false>(ofs,100000,1000);
		ofs<<"__gnu_cxx::rope<int>\n";
		SingleOperationPerformanceCheck<__gnu_cxx::rope<int> ,false>(ofs,100000,1000);
		ofs<<"alex_kupriianov_array<int>\n";
		SingleOperationPerformanceCheck<alex_kupriianov_array<int,MMM,NNN> ,false>(ofs,1,10000000);
		ofs<<"__gnu_cxx::rope<int>\n";
		SingleOperationPerformanceCheck<__gnu_cxx::rope<int> ,false>(ofs,1,10000000);
	}
}
